...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <func:function name="keg:translateInterestMeasure">
    ...
    <xsl:variable name="translated" >
      <xsl:choose>
        <xsl:when test="$type='InterestMeasure'">
          <xsl:value-of select="exsl:node-set($InterestMeasureDictionary)/InterestMeasures/InterestMeasure[str/@lang='pmml' and str=$name]/str[@lang=$toLang]"/>
        </xsl:when>
        <xsl:when test="$type='TestCriterion'">
          <xsl:value-of select="exsl:node-set($InterestMeasureDictionary)/InterestMeasures/InterestMeasure/TestCriteria/TestCriterion[str/@lang=$fromLang and str=$name]/str[@lang=$toLang]"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="$translated and not($translated='')"> <func:result select="concat($translated, ' ')"/></xsl:when>
      <xsl:otherwise><func:result select="concat($name, ' ')"/></xsl:otherwise>
    </xsl:choose>
  </func:function>
  ...
  <func:function name="keg:selectQuantifier">
    ...
    -->
    <xsl:variable name="Symbol" select="exsl:node-set($InterestMeasureDictionary)//InterestMeasure[str[@lang='MasterName']=$quantifierName]/Symbol/node()"/>
