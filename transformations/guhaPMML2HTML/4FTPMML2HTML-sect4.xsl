...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="DBASetting" mode="sect4rule">
    ...
    <xsl:choose>
      <xsl:when test="@type='Literal' and LiteralSign='Both'">
        ...
        <xsl:value-of select="$orOperator"/> <!-- V -->
        <xsl:value-of select="$notOperator"/><!-- NOT -->
        <xsl:apply-templates select="BASettingRef" mode="sect4rule">
          <xsl:with-param name="link" select="$link"/>
        </xsl:apply-templates>)
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="@type='Literal' and LiteralSign='Negative'"> <xsl:value-of select="$notOperator"/><!-- NOT --></xsl:if>
        <xsl:apply-templates select="BASettingRef" mode="sect4rule">
          <xsl:with-param name="link" select="$link"/>
        </xsl:apply-templates>
      </xsl:otherwise>
  ...
  <xsl:template match="BASettingRef" mode="sect4rule">
    ...
    <xsl:if test="position() > 1">
      <xsl:choose>
        <xsl:when test="../@type='Disjunction'"> <xsl:value-of select="$orOperator"/> <!-- V --></xsl:when>
        <xsl:when test="../@type='Conjunction'"> <xsl:value-of select="$andOperator"/> <!-- & --></xsl:when>
      </xsl:choose>
    </xsl:if>
    <!--  derived attribute consist of references to other basic or derived attributes - RECURSION -->
    <xsl:apply-templates select="../../../BBASettings/BBASetting[@id=$ref] | ../../DBASetting[@id=$ref]" mode="sect4rule">
      <xsl:with-param name="link" select="$link" />
    </xsl:apply-templates>
  </xsl:template>
  <!-- END: rule generation -->
  ...
  <xsl:template match="BBASetting" mode="sect4">
    <tr>
      ...
      <td><xsl:value-of select="Coefficient/Type"/></td>
      <td><xsl:value-of select="Coefficient/MinimalLength"/></td>
      <td><xsl:value-of select="Coefficient/MaximalLength"/></td>
    </tr>
  </xsl:template>
  ...
  <xsl:template match="DBASetting" mode="sect4">
    <tr>
      ...
      <td><xsl:value-of select="MinimalLength"/></td>
      <td><xsl:value-of select="MaximalLength"/></td>
    </tr>
  </xsl:template>
  <!-- END: table rows -->
