...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="p:TransformationDictionary" mode="sect3list">
    ...
    <table class="listTable">
      ...
      <tr>
        <th scope="col">
          <xsl:copy-of select="keg:translate('Attribute',360)"/>
        </th>
        <th scope="col">
      ...
      <div class="details">
            ...
            <th scope="col">
              <xsl:choose>
                <xsl:when test="p:Discretize">
                  <xsl:copy-of select="keg:translate('Interval',390)"/>
                </xsl:when>
                <xsl:when test="p:MapValues">
                  <xsl:copy-of select="keg:translate('Enumeration',400)"/>
                </xsl:when>
              </xsl:choose>
            </th>
            <th scope="col">
          ...
          <xsl:if test="$numOfCategories > $maxCategoriesToList">
            <tr>
              <td colspan="2" class="name others">
                <xsl:copy-of select="keg:translate('Other categories',801)"/>
        ...
        <xsl:if test="$numOfCategories > $maxCategoriesToList">
          <p class="warning">
            ...
            <xsl:value-of select="$numOfCategories"/>
            <xsl:copy-of select="keg:translate('categories',290)"/>.
          </p>
        </xsl:if>
      </div>
    </section>
