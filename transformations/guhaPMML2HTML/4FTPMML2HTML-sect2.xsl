...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="p:DataField" mode="sect2">
    ...
    <section id="sect2-{@name}" class="dataField">
      ...
      <h3><xsl:copy-of select="keg:translate('Column',251)"/>:
      ...
      <table class="fieldBasicInfo">
        <tr>
          <th scope="row">
            <xsl:copy-of select="keg:translate('Number of different values',260)"/>
          </th>
          <td>
            <xsl:choose>
        ...
        <tr>
          <th scope="row">
            <xsl:copy-of select="keg:translate('Type',270)"/>
          </th>
          <td>
        ...
        <xsl:if test="count(p:Interval)>0">
          <tr>
            <th scope="row">
              <xsl:copy-of select="keg:translate('Value range',311)"/>
            </th>
            <td>
  ...
  <xsl:template match="p:Interval" mode="sect2">
    <xsl:choose>
      <xsl:when test="@closure='closedOpen' or @closure='closedClosed'">
        <xsl:copy-of select="keg:translate('[',1)"/>
      </xsl:when>
      <xsl:otherwise>(</xsl:otherwise>
    </xsl:choose>
    <xsl:value-of select="@leftMargin"/>;<xsl:value-of select="@rightMargin"/>
    <xsl:choose>
      <xsl:when test="@closure='closedClosed' or @closure='openClosed'">
  ...
  <xsl:template match="p:DataDictionary" mode="sect2list">
    ...
    <table>
      ...
      <tr>
        <th scope="col">
          <xsl:copy-of select="keg:translate('Column name',250)"/>
        </th>
        <th scope="col">
          <xsl:copy-of select="keg:translate('Number of diffrent values',260)"/>
        </th>
        <th scope="col">
          <xsl:copy-of select="keg:translate('Type',270)"/>
        </th>
