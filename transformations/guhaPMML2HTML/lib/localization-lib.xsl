...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <func:function name="keg:translate">
    ...
    <xsl:variable name="translated"
                  select="exsl:node-set($LocalizationDictionary)/Dictionary/Entry[str/@lang=$from and (@id=$id or str=$name)]/str[@lang=$reportLang]"></xsl:variable>
    ...
    <xsl:variable name="visualization"
                  select="exsl:node-set($LocalizationDictionary)/Dictionary/Entry[str/@lang=$from and (@id=$id or str=$name)]/Visualization"></xsl:variable>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <func:result select="$outputString"/>
      </xsl:otherwise>
    </xsl:choose>
  </func:function>
